...
/*
 ...
 --------------------------------------------------------------------
 */
int prepare_new_file_name(char *fname)
{
	char *newfname = 0;
	int len;
	...
	if (len < 0) {
		printf("is_translit_need() failed %d\n", len);
		return -1;
	}

	if (len > 0) {
		printf("File name after translit = %d\n", len);
		newfname = malloc(len);
		translit_utf8(fname, newfname);
		printf("File name after translit: %s\n", newfname);

	} else {
		printf("No need to translit file name\n");
	}
